...
{{
  ...
  "since": "YYYY-MM-DD"
            ...
            print(f"Attempt {attempt + 1} - Raw response preview: {text[:300]}")
            ...
            if not text or not text.startswith("["):
                print(f"Attempt {attempt + 1}: No JSON array found, retrying in 5s...")
                time.sleep(5)
                continue

            zones = json.loads(text)
            if not zones:
                print(f"Attempt {attempt + 1}: Empty array, retrying in 5s...")
                time.sleep(5)
                continue

            print(f"Attempt {attempt + 1}: Successfully parsed {len(zones)} zones")
            return zones

        except json.JSONDecodeError as e:
            print(f"Attempt {attempt + 1}: JSON parse error: {e}, retrying in 5s...")
            time.sleep(5)
        except Exception as e:
            print(f"Attempt {attempt + 1}: Unexpected error: {e}, retrying in 5s...")
            time.sleep(5)
...
def refresh_bigquery(zones):
    ...
    if load_job.errors:
        ...
        for z in zones:
            print(f"  {z['severity']:12} | {z['country']:35} | {z['reason'][:60]}")
